...
const FeedVsMilkYieldChart = ({farmId}) => {
    ...
    (async () => {
      try {
        ...
        const sanitized = (raw || []).filter(entry => {
          const feed = Number(entry?.feed_consumption);
          const milk = Number(entry?.milk_yield);
          return (
            entry?.date &&
            isFinite(feed) &&
            isFinite(milk) &&
        ...
        const formatted = sanitized.map(item => ({
          date: item.date,
          feed: Number(item.feed_consumption),
          milk: Number(item.milk_yield),
        }));

        setData(formatted);
      } catch (err) {
        console.error('FeedVsMilkYieldChart error:', err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const feedData = data.map(d => d.feed);
  const milkData = data.map(d => d.milk);

  // Each cluster gets 80px
  const clusterWidth = 80;
  const chartWidth = Math.max(labels.length * clusterWidth, screenWidth);
